...
- Long AI calls run in `JobRunner` (a `BackgroundService`), serially. Never call the API from a component: a multi-minute request would hold the Blazor circuit. Jobs interrupted by a restart are marked Failed at startup rather than left spinning.
- Auth activates only when `StudyApp__Password` is set, so local runs stay friction-free. **Any deployment carrying an API key must set it** — an open instance doesn't merely leak flashcards, it lets a stranger spend money on generation runs.
